...
/*   Updated: 2025/09/02 15:12:35 by xalves           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	send_str(char *chr, int pid)
{
	int	i;
...
static int	send_len(char *str, int pid)
{
	int	i;
	int	len;
...
int	main(int argc, char **argv)
{
	...
	if (pid < 1 || kill(pid, 0) == -1)
		return (ft_printerror("Invalid PID, ola\n"), 1);
	if(send_len(argv[2], pid) == 1)
		return (1);
	i = 0;
	if (!argv[2][i])
		send_str("", pid);
